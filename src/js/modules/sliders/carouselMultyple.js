const carouselMultyple = (selector, navigationType = 'dots') => {
    // ===== DOM ЭЛЕМЕНТЫ =====
    ...
    // Проверяем, какие элементы навигации доступны в верстке
    const hasButtons = prevBtn && nextBtn;
    const hasDots = dotsContainer;
    
    // Проверяем соответствие запрошенного типа тому, что есть в верстке
    if (navigationType === 'btns' && !hasButtons) return;
    
    if (navigationType === 'dots' && !hasDots) return;
    ...
    const updateVisibleSlides = () => {
        ...
        const newVisibleSlides = parseInt(sliderStyle.getPropertyValue('--visible-slides')) || 
                                parseInt(getComputedStyle(document.documentElement).getPropertyValue('--visible-slides')) || 
                                3;
    ...
    const updateSliderPosition = () => {
        ...
        const gap = parseInt(getComputedStyle(field).gap) || 0;
    ...
    const handleButtonClick = (direction) => {
        if (isAnimating || !hasButtons) return;
        isAnimating = true;
    ...
    const init = () => {
        ...
        // Создаем индикаторы, если они есть в верстке
        if (hasDots) {
            createIndicators();
        }
...
//     // ===== АДАПТИВНОСТЬ =====
//     const updateVisibleSlides = () => {
//         // Получаем переменную от самого слайдера (приоритет)
//         const sliderStyle = getComputedStyle(slider);
//         const newVisibleSlides = parseInt(sliderStyle.getPropertyValue('--visible-slides')) || 
//                                 // Fallback к :root если не задано
//                                 parseInt(getComputedStyle(document.documentElement).getPropertyValue('--visible-slides')) || 
//                                 3; // Default fallback
